...
class Solver:
    ...
    def learn(self, optimizers, epochs, n_sample, save_dir):
        ...
        heading = "{:>6}{:>12}{:>18}".format('epoch', 'loss_main', 'runtime(s)')
        print(heading)
        start = time.time()
        ...
        with open('{}/training_log.txt'.format(save_dir), 'w') as log:
            ...
            for epoch in range(epochs):
                ...
                l1, bdry_loss, engy_loss = self.train_step_main(optimizers[0], domain_data, boundary_data)
                ...
                losses[epoch, :] = np.array([l1.numpy(), curl_loss.numpy(), bdry_loss.numpy(),\
                               engy_loss.numpy(), div_loss.numpy()])
                if epoch % 10 == 0:
        ...
        header=['main', 'curl', 'boundary', 'energy', 'div'])
